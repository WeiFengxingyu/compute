...
- [ ] PSO 版本：将每线每时段产量比例与产品选择概率连续化；解码器保持一致。
- [ ] ACO 版本：构造时段-产品序列；信息素奖励高利润密度与早完工；蒸发系数与启发项调优（可选）。
- [ ] 与 GA/GA+VNS/GA+SA/TS 对比：同场景与种子集合做 30 次重复。

执行命令示例：
```
python -m src.main --horizon 7 --generations 300 --pop 120 --pc 0.8 --pm 0.1 \
  --config data/config.json --orders data/orders.json --out results \
  --runs_dir experiments --exp_tag pso-or-aco --seed 606
```
...
- [ ] **阶段2（完整对比）**（如果时间允许）：
  - 场景设定：
    - 工资系数（低/中/高），例如：
      - 低：`[1.0, 1.05, 1.1, 1.2, 1.3, 1.15]`
      - 中：`[1.0, 1.1, 1.2, 1.35, 1.5, 1.3]`（默认）
      - 高：`[1.0, 1.15, 1.3, 1.5, 1.7, 1.4]`
    - 订单紧迫度（宽松/中等/紧张）：通过到期分布与数量控制。
  - 每个场景×算法组合30次重复
