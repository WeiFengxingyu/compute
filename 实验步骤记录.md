...
## M7 阶段：强化适应度函数与PSO算法实现

### M7.1 强化适应度函数（2025-01-01）
- 目的：解决准时率偏低（接近0%）问题，通过增强软惩罚引导算法重视截止日期
- 修改文件：`src/evaluation/fitness.py`
- 主要改进：
  - 增加`alpha_deadline`从0.5到1.5，`beta_late_units`从0.2到0.8
  - 截止日期压力计算改为分段函数，对超期项目采用指数增长惩罚
  - 延迟单位惩罚按订单价值加权，高价值订单延迟惩罚更重
- 测试结果：为后续PSO算法提供更好的适应度引导

### M7.2 实现粒子群优化（PSO）算法（2025-01-01）
- 目的：增加算法对比，丰富论文实验内容，验证不同元启发式算法效果
- 新增文件：`src/algorithms/pso.py`
- 算法设计：
  - 编码：连续空间，每个粒子维度对应订单优先级（[0,1]范围）
  - 解码：基于优先级的EDD + 利润密度解码器
  - 适应度：使用强化后的软适应度函数
  - 参数：30粒子，200迭代，c1=c2=2.0，w从0.9线性递减到0.5
- 集成文件：
  - `src/main.py`：添加PSO参数和算法选择
  - `src/experiments/runner.py`：添加PSO实验支持
- 初步测试：
  - 命令：`python -m src.main --pso_enabled --pso_particles 20 --pso_iterations 50 --soft_deadline --soft_alpha 1.5 --soft_beta 0.8 --exp_tag test-pso --seed 42`
  - 结果：利润48,180，按时完成率75%，罚款比例25%
  - 对比：显著优于GA（通常按时完成率~0%）
- 下一步：进行完整对比实验（30次运行），测试不同订单紧迫度场景

注：批量运行器 `src/experiments/runner.py` 属于后续批量实验（计划M6），我们已提前实现用于验证与统计，其参数会自动映射到 `src.main` 的短旗标。

## M8 阶段：不同订单紧迫度场景对比实验（2025-01-02）

### M8.1 场景数据生成
- 目的：创建不同订单紧迫度场景（宽松loose、中等medium、紧迫tight）用于算法对比
- 新增文件：`src/data/scenario_generator.py`
- 主要功能：
  - 根据紧迫度参数调整订单交付期限分布
  - 生成对应场景的配置和订单数据
  - 保存为JSON格式供后续实验使用
- 生成结果：
  - `data/scenarios/orders_loose.json`：平均交付期限2.9天
  - `data/scenarios/orders_medium.json`：平均交付期限1.4天
  - `data/scenarios/orders_tight.json`：平均交付期限1.4天
  - 对应的配置文件：`config_loose.json`、`config_medium.json`、`config_tight.json`

### M8.2 场景对比实验运行
- 目的：在所有场景下运行 GA、GA-VNS、GA-VNS-SA、PSO 算法各 30 次
- 运行命令：`python -m src.experiments.scenario_runner`
- 实验规模：3 场景 × 4 算法 × 30 次 = 360 次实验
- 结果目录：`experiments/run-*-loose-m6s3/`、`experiments/run-*-medium-m6s3/`、`experiments/run-*-tight-m6s3/`
- 状态更新（2025-01-02）：已完成聚合与统计，详见下文 M8.3

### M8.3 数据聚合与统计输出（2025-01-02）
- 目的：将三场景×四算法×30 次运行的 `metrics.json` 聚合为批量 CSV 并生成统计表与图表
- 脚本位置：`experiments/scripts/aggregate_m8_batches.py`、`compute_m8_averages.py`、`plot_m8_figures.py`
- 操作步骤：
  1) `python experiments/scripts/aggregate_m8_batches.py`（生成/更新 `experiments/batch_*_summary.csv`）
  2) `python experiments/scripts/compute_m8_averages.py`（计算均值±标准差并打印到控制台）
  3) `python experiments/scripts/plot_m8_figures.py`（输出四张图与总结表到 `experiments/m8_plots/`）
- 验收：`experiments/m8_plots/m8_summary_table.md` 与四张图存在，样本数均为 30（每算法每场景）；若 PSO 某场景标准差为 0，属当前设置下的稳定最优表现。

### PSO 随机种子验证与重跑策略
- 随机种子：`src/main.py` 在入口处调用 `random.seed(args.seed)`，PSO 使用 `random.uniform/random.random`，已受 `--seed` 控制。
- 若需增强随机性或区分不同种子的表现：可在 `src/algorithms/pso.py` 的优先级或槽位吸引度排序中加入微小随机抖动，或调整 `w/c1/c2/max_vel` 等参数扩大搜索空间。
- 是否需要重做 M8：
  - 不改算法逻辑/参数时：无需重做，当前聚合与统计已完成。
  - 修改 PSO 随机性或参数后：仅重跑 PSO 的三场景×30 次即可（GA/GA+VNS/GA+VNS+SA 保持不变）。
- 重跑 PSO 示例（按场景）：
  - `python -m src.experiments.runner --exp_tag pso-loose-m6s3 --scenario loose --append_scenario_to_tag --pso_enabled --pso_particles 20 --pso_iterations 50 --soft_deadline --soft_alpha 1.5 --soft_beta 0.8 --seed-start 2000 --seed-end 2029 --runs_dir experiments --outdir results`
  - `python -m src.experiments.runner --exp_tag pso-medium-m6s3 --scenario medium --append_scenario_to_tag --pso_enabled --pso_particles 20 --pso_iterations 50 --soft_deadline --soft_alpha 1.5 --soft_beta 0.8 --seed-start 2000 --seed-end 2029 --runs_dir experiments --outdir results`
  - `python -m src.experiments.runner --exp_tag pso-tight-m6s3 --scenario tight --append_scenario_to_tag --pso_enabled --pso_particles 20 --pso_iterations 50 --soft_deadline --soft_alpha 1.5 --soft_beta 0.8 --seed-start 2000 --seed-end 2029 --runs_dir experiments --outdir results`
  - 然后按 M8.3 步骤更新聚合与图表。

### M8 复盘与关键结论（更新）
- 强化适应度函数是解决低准时率的关键：提高截止权重与软惩罚系数（例如提升 `alpha_deadline`、`beta_late_units`），三场景统计均验证其对提升准时率与降低罚金触发率的显著作用（见 `experiments/m8_plots/m8_summary_table.md`）。
- 横向对比算法采用 PSO：在现有框架下实现并接入批量实验，作为论文的横向对比算法；PSO 的随机源受 `--seed` 控制。如需调整 PSO 的随机性或参数（`w/c1/c2/max_vel`），仅重跑 PSO 三场景×30 次，并按 M8.3 重新聚合与绘图，GA/GA+VNS/GA+VNS+SA 无需重跑。
- 数据与图表位置：`experiments/m8_plots/`（含 `m8_summary_table.md` 与四张图），并已同步报告到 `paper/figures/m8_plots/m8_experiment_report.md`。
- 验收更新：所有算法在三场景的样本数均为 30；若某场景标准差为 0，视为该设置下的稳定表现而非统计缺失。
...
- 说明：均为每算法每场景 n=30 的样本，均值±标准差由 `experiments/scripts/compute_m8_averages.py` 输出；显著性为“与 GA 基准的配对 t 检验”，由 `experiments/scripts/compute_m8_tests.py` 输出（ns: 不显著；*: p<0.05；***: p<0.001）。
